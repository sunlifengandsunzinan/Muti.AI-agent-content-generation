...
采集完成后，检查作品列表中 **发布已超过24h但播放<300** 的视频。

对每个符合条件的视频，尽可能从创作者中心详情页获取：
- 完播率
- 2s跳出率
- 流量来源分布（推荐/搜索/主页等）
- 粉丝播放占比

如果详情页有数据，按以下格式分析原因并写入 `data.json -> analyses.lowPlayback`：

```json
{
  "videoTitle": "视频标题",
  "playCount": 251,
  "reason": "分析出的核心原因（封面/标题/限流/发布时间等）",
  "detail": "一句话详细说明"
}
```

分析维度：
1. **封面问题** — 无人物/无文字/反差不够 → 点击率低
2. **标题问题** — 陈述句/太平淡/无钩子 → 信息流不戳
3. **定时发布故障** — 发布状态异常 → 错过初始推荐
4. **同天撞车** — 同一天发了多条 → 推流分流
5. **内容方向** — 非粉丝想看的内容类型 → 推荐不精准
6. **限流标记** — 内容触发审核降权

如果详情页没有数据（数据暂未产出）→ 标记 pending，下次采集时复查。
