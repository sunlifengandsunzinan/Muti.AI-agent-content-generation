...
**核心公式：每千播涨粉率 = 涨粉数 / 播放数 × 1000**
健康值：2-5/千播 | 当前：0.28/千播 | 差距：1/7~1/18

| 视频 | 播放 | 完播 | 涨粉 | 转化率/千播 | 原因 |
|:----|:---:|:---:|:---:|:----------:|:----|
| 标题 | xxx | xx% | +x | x.xx | 结尾有/无身份锚点 |

**涨粉趋势：**
- 24h变化：-x粉
- 7d变化：+x粉
- 连续零涨粉天数：x天 ← 如果>3天高亮告警
...
基于`skills/douyin-情报站/SKILL.md`的账号诊断流程六大维度，综合给出以下输出：

```
基于当前数据，建议下一步：
- 方向建议：____（基于涨的趋势 + 素材可用状态）
- 素材建议：____（推荐哪个素材+哪个角度）
- 风险提示：____（观点重复/素材禁用/赛道偏离等）
- 禁用提示：____（明确不能发什么方向）
```

**如果当天需要给方向建议，调用账号诊断流程读全量数据，不要只看简报数据。**
